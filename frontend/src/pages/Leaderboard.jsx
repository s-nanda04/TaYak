...
export default function Leaderboard() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/leaderboard`)
      ...
      .catch((e) => {
        console.error(e);
        setError(e.message || "Could not load leaderboard. Is the backend running?");
        setLoading(false);
      });
  }, []);
          ...
          <div className="flex items-center justify-between h-14">
            <div className="flex items-center gap-3.5">
              <button
                ...
                className="flex flex-col gap-1 p-1 group"
              >
          ...
          <>
            <Section title="Top Contributors">
              {!data.top_contributors?.length ? (
                <p className="text-caption text-txt-tertiary px-4 py-6 text-center">No posts yet.</p>
              ) : (
                data.top_contributors.map((p, i) => (
                  <Row
                    ...
                    right={`${p.posts} posts`}
                    index={i}
                    total={data.top_contributors.length}
                  />
                ))
              )}
            </Section>
            ...
            <Section title="Top Topics">
              {!data.top_topics?.length ? (
                <p className="text-caption text-txt-tertiary px-4 py-6 text-center">No topics yet.</p>
              ) : (
              ...
              {!data.top_posts?.length ? (
                ...
                data.top_posts.map((post, i) => (
                  <div
                    key={post.id}
                    className={`px-4 py-3 ${
                      i < data.top_posts.length - 1 ? "border-b border-subtle" : ""
                    }`}
                  >
