...
const CommentCard = ({ comment, isLast }) => (
    ...
    <div className="flex items-start gap-3">
      <div
        ...
        style={{
          ...
          borderColor: `${comment.color}30`,
        }}
      >
        {comment.profile_pic}
      </div>
      <div className="flex-1 min-w-0">
        <span className="text-caption text-txt-tertiary">{comment.time} ago</span>
...
export default function CommentsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${API}/yaks`, { headers }).then(r => r.json()),
      fetch(`${API}/yaks/${id}/comments`).then(r => r.json()),
    ])
      .then(([yaks, commentsData]) => {
        const found = yaks.find(y => y.id === id);
        setYak(found || null);
  ...
  const handleVote = async (yakId, voteValue, delta) => {
    ...
    try {
      ...
      if (!res.ok) {
        setYak(prev => prev ? { ...prev, votes: prev.votes - delta, user_vote: voteValue - delta } : prev);
      }
    } catch {
      setYak(prev => prev ? { ...prev, votes: prev.votes - delta, user_vote: voteValue - delta } : prev);
    }
      ...
      {composing && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center">
          <div
            ...
            onClick={() => { if (!posting) { setComposing(false); setCommentText(""); } }}
          />
          <div className="relative w-full sm:max-w-[480px] bg-card border border-[#ECEDEF] sm:rounded-md rounded-t-md shadow-card mx-0 sm:mx-4 animate-toast-in">
            <div className="flex items-center justify-between px-5 pt-4 pb-2">
              ...
              <span className="font-semibold text-body-md text-txt-primary">New Comment</span>
              <button
